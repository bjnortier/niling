  ...
  socket.on('put_obj', function(msg) {
    ...
    container.putObject(object, {silent: true}, function(err) {
      if (err) {
        ...
        _this.emit('synced', from, to, {type: 'put_obj', hash: hash, object: object});
        socket.emit('ack', from, to, {type: 'put_obj', hash: hash});
      }
    });
  });

  socket.on('ack', function(from, to, syncResult) {
    _this.emit('ack', from, to, syncResult);
  }); 
  ...
  socket.on('put_ref', function(msg) {
    ...
    container.putReference(msg.key, object, {silent: true}, function(err, result) {
      if (err) {
        ...
        _this.emit('synced', from, to, {type: 'put_ref', key: msg.key, object: object});
      }
    });
  });
  ...
  socket.on('update_ref', function(msg) {
    ...
    container.updateReference(msg.key, object, msg.prevVsn, {silent: true}, function(err, result) {
      if (err) {
        ...
        _this.emit('synced', from, to, {type: 'update_ref', key: msg.key, object: object});
      }
    });
  });
